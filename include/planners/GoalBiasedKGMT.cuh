...
public:
    ...
    GoalBiasedKGMT(float width, float height, int N, int n, int numIterations, int maxTreeSize, int numDisc, float agentLength,
                   float goalThreshold);
    ...
    bool* h_finalizedVertices_;
    thrust::device_vector<bool> d_G_;  // Set of samples to be expanded in current iteration.
    thrust::device_vector<bool> d_GNew_;
    thrust::device_vector<int>
      d_scanIdx_;  // stores scan of G. ex: G = [0, 1, 0, 1, 1, 0, 1] -> scanIdx = [0,0,1,1,2,3,3]. Used to find active samples in G.
    ...
    thrust::device_vector<int> d_vertexArray_;
    thrust::device_vector<int> d_edgeArray_;
    thrust::device_vector<float> d_weightArray_;
    thrust::device_vector<bool> d_finalizedVertices_;
    ...
    int* d_predNode_ptr_;
    int* d_vertexArray_ptr_;
    int* d_edgeArray_ptr_;
    float* d_weightArray_ptr_;
    bool* d_finalizedVertices_ptr_;
    float* d_shortestDistances_ptr_;
    float* d_updatingShortestDistances_ptr_;
...
propagateGV2_gb(int sizeG, int* activeGIdx, bool* G, bool* GNew, float* treeSamples, float* unexploredSamples, int* uParentIdx,
                ...
                curandState* randomStates, int numDisc, float agentLength, float* R1Threshold, float* R1Scores, float* obstacles,
                int obstacleCount, int iterations, float width, float height, int* selR1Edge, int* valR1Edge);

__global__ void updateR1_gb(float* R1Score, int* R1Avail, int* R2Avail, int* R1Valid, int* R1Invalid, int* R1Sel, int n, float epsilon,
