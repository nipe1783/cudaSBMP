...
// Global: called from the CPU to the GPU
__global__ 
void generateRandomTreeKernelV2(const float* root, float* tree, const int numIterations, int tWidth) {
...
void Planner::generateRandomTreeV2(const float* root, const int numSamples, float **samples){
    // initialize execution parameters
    ...
    // Call the kernel
    generateRandomTreeKernelV2<<<dimGrid, dimBlock>>>(dRoot, dTree, rowsTree, tWidth);
