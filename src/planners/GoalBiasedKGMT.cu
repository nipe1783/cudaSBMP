    ...
    , goalThreshold_(goalThreshold)
{
    ...
    initHashMap<<<(nR1Edges_ + 255) / 256, 256>>>(
      thrust::raw_pointer_cast(d_fromNodes_.data()), thrust::raw_pointer_cast(d_toNodes_.data()),
      thrust::raw_pointer_cast(d_edgeIndices_.data()), thrust::raw_pointer_cast(d_hashTable_.data()), tableSize_, nR1Edges_);
    ...
    thrust::copy(vertexArray.begin(), vertexArray.end(), d_vertexArray_.begin());
    thrust::copy(edgeArray.begin(), edgeArray.end(), d_edgeArray_.begin());
    thrust::copy(weightArray.begin(), weightArray.end(), d_weightArray_.begin());
    ...
    while(itr < numIterations_)
        {
            ...
            updateR_gb<<<gridSizeActive, NUM_R1_CELLS>>>(d_R1Score_ptr_, d_R2Avail_ptr_, d_R1Avail_ptr_, d_R1Valid_ptr_, d_R1_ptr_, n_,
                                                         ...
                                                         d_selR1Edge_ptr_, d_valR1Edge_ptr_, d_R1Threshold_ptr_);

            dijkstraGPU(r1_initial, NUM_R1_CELLS, nR1Edges_, d_vertexArray_ptr_, d_edgeArray_ptr_, d_weightArray_ptr_, d_finalizedVertices_ptr_,
                        d_shortestDistances_ptr_, d_updatingShortestDistances_ptr_, h_finalizedVertices_);
            ...
            if(blockSizeActive * gridSizeActive > maxTreeSize_ - treeSize_)
                {
                    ...
                    propagateGV2_gb<<<gridSizeActive, blockSizeActive>>>(
                      ...
                      d_randomStates, numDisc_, agentLength_, d_R1Threshold_ptr_, d_R1Score_ptr_, d_obstacles, obstaclesCount, iterations,
                      width_, height_, d_selR1Edge_ptr_, d_valR1Edge_ptr_);
                }
            else
                {
                    propagateG_gb<<<gridSizeActive, blockSizeActive>>>(
                      activeSize, d_activeIdx_ptr_, d_G_ptr_, d_GNew_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, d_uParentIdx_ptr_,
                      d_R1Valid_ptr_, d_R2Valid_ptr_, d_R1_ptr_, d_R2_ptr_, d_R1Avail_ptr_, d_R2Avail_ptr_, N_, n_, R1Size_, R2Size_,
                      d_randomStates, numDisc_, agentLength_, d_R1Threshold_ptr_, d_R1Score_ptr_, d_obstacles, obstaclesCount, width_,
                      height_, d_selR1Edge_ptr_, d_valR1Edge_ptr_, d_hashTable_ptr_, tableSize_);
                }
